...
def por_responsavel(
    ...
    resultado = [t for t in banco if nome.lower() in (t.responsavel or '').lower()] # "t": variável para tarefa
    if not resultado:
        raise HTTPException(status_code=404, detail=f'Nenhuma tarefa encontrada')
    return resultado

# GET /prioridade/critica
    ...
    for i, t in enumerate(banco):
        if t.id == tarefa_id:
            if t.status == StatusEnum.cancelada and dados.status != StatusEnum.cancelada:
                raise HTTPException(status_code=400, detail='Tarefas canceladas não podem ser alteradas')
            atual = t.model_dump()
